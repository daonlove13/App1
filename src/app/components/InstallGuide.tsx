...
export default function InstallGuide({ onSkip }: Props) {
  ...
  // 이미 설치돼서 standalone으로 실행 중이면 바로 진행
  useEffect(() => {
  ...
  const iosSteps = [
    {
      icon: '↑',
      title: '하단 공유 버튼 탭',
      desc: 'Safari 하단 가운데에 있는 □↑ 버튼을 누르세요',
    },
    {
      icon: '+',
      title: '"홈 화면에 추가" 선택',
      desc: '스크롤을 내려 "홈 화면에 추가"를 찾아 탭하세요',
    },
    {
      icon: '✓',
      title: '"추가" 탭',
      desc: '오른쪽 상단 "추가"를 탭하면 아이콘이 생겨요',
    },
  ];
  ...
  const androidSteps = [
    {
      icon: '⋮',
      title: '우측 상단 메뉴 탭',
      desc: 'Chrome 우측 상단 점 세 개(⋮) 버튼을 누르세요',
    },
    {
      icon: '+',
      title: '"홈 화면에 추가" 선택',
      desc: '"홈 화면에 추가" 또는 "앱 설치"를 탭하세요',
    },
    {
      icon: '✓',
      title: '"추가" 탭',
      desc: '확인 버튼을 탭하면 홈 화면에 아이콘이 생겨요',
    },
  ];
          ...
          {steps.map((s, i) => (
            <div
              ...
              className={`flex items-start gap-4 p-4 rounded-2xl transition-all ${
                step === i ? 'bg-white/10 border border-white/20' : 'bg-white/5'
              }`}
            >
          ...
          {steps.map((_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                step === i ? 'bg-white w-6' : 'bg-gray-600'
              }`}
            />
          ))}
        </div>
